...
class MultimodalModel(BertPreTrainedModel):
    ...
    def forward(self, image_input=None, text_input=None):
        if (image_input is not None) and (text_input is not None):
            ...
            image_text_attention_state, _ = self.comb_attention(
                image_text_hidden_state.transpose(0, 1),
                image_text_hidden_state.transpose(0, 1),
                image_text_hidden_state.transpose(0, 1),
                key_padding_mask=attention_mask
            )
            image_text_attention_state = image_text_attention_state.transpose(0, 1)
